...
class CasperSplashTests: XCTestCase {
    ...
    func testAddIcon_CheckIfNSImage() {
        let path = Bundle(for: type(of: self)).pathForImageResource(NSImage.Name(stringLiteral: "ec_32x32.png"))
    ...
    func testAddIcon_WithIncorrectResource() {
        let path = Bundle(for: type(of: self)).pathForImageResource(NSImage.Name(stringLiteral: "nonexistent"))

        XCTAssertEqual(Software(packageName: "EC", version: "1.6.2", status: .installing, iconPath: path).icon, NSImage(named: NSImage.folderName))
    }
